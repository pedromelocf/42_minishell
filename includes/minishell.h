...
# define HEREDOC_WARNING "zapshell :warning: here-document at line\
...
char					*ft_strjoind(char *first, char *second,
							char *delimiter);
void					set_parse_status(t_parse_status *status,
							enum e_parse_status new_status,
							t_dlist *word);
...
char					*get_env_var(char *cmd, size_t start, size_t *index,
							char **env);
...
void					set_env_paths(char *old_path, char *new_path,
							char ***env);
