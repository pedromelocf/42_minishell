...
/*   Updated: 2024/09/19 16:41:15 by pmelo-ca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	redirect_in(t_redir *tmp_redirs, bool piped)
{
	...
	if (!tmp_redirs->expanded_values || !tmp_redirs->expanded_values[0])
		return (ft_dprintf(2, ERROR_AMBIGUOUS_REDIRECT, tmp_redirs->value), 1);
	...
	if (fd == -1)
	{
		...
		{
			if (piped)
				return (ft_dprintf(2, ERROR_EXEC_INVALID_PATH,
						tmp_redirs->value), 257);
			return (ft_dprintf(2, ERROR_EXEC_INVALID_PATH, tmp_redirs->value),
				1);
		}
		if (piped)
			return (ft_dprintf(2, ERROR_EXEC_PERMISSION_DENY,
					tmp_redirs->value), 257);
		return (ft_dprintf(2, ERROR_EXEC_PERMISSION_DENY, tmp_redirs->value),
			1);
	}
